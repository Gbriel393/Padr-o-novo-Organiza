          ...
          <tbody>
            ...
            <?php foreach ($transacoes as $t): $entrada = $t['tipo'] === 'entrada';
              ...
              <tr id="registro-<?= (int) $t['id'] ?>" class="<?= (int) $t['id'] === $registroCriado ? 'table-success' : '' ?>" title="<?= e((string) $t['descricao']) ?>">
                ...
                <td>
                  <form method="post" class="form-excluir" onsubmit="return confirm('Excluir esta transação?');"><input type="hidden" name="csrf_token" value="<?= e($_SESSION['csrf_token']) ?>"><input type="hidden" name="acao" value="excluir"><input type="hidden" name="id" value="<?= (int) $t['id'] ?>">
                    <button type="submit" class="acao-excluir btn btn-danger" title="Excluir" aria-label="Excluir">
                      ✖
                    </button>

                    <!-- <button type="button" class="acao-excluir btn btn-danger" title="Excluir" aria-label="Excluir">Danger</button> -->
